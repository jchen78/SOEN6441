...
public class MapAreaTests {
	...
	@Test public void GivenMapAreaWithTroubleMarkerAlreadySetWhenAddingBuildingShouldThrowException() throws InvalidOperationException {
		MapArea mapArea = new MapArea();
		mapArea.addMinions(new Player(0), 2);
		InvalidOperationException expectedException = null;
	...
	@Test public void WhenAddingMinionMapAreaShouldUpdateItself() throws InvalidOperationException {
		MapArea mapArea = new MapArea();
		mapArea.addMinions(new Player(1), 4);
	...
	try { mapArea.addMinions(new Player(1), 4); } catch (InvalidOperationException e) { expectedException = e; }
	...
	try { mapArea.addMinions(new Player(5), 4); } catch (InvalidOperationException e) { expectedException = e; }
	...
	@Test public void WhenAddingMinionWithNegativeNumberOfMinionsShouldThrowException() {
		...
		try { mapArea.addMinions(new Player(2), -1); } catch (InvalidOperationException e) { expectedException = e; }
	...
	@Test public void WhenAddingMinionWithTooManyMinionsShouldThrowException() {
		...
		try { mapArea.addMinions(new Player(2), 13); } catch (InvalidOperationException e) { expectedException = e; }
